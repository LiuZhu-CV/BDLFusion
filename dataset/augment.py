...
class Transforms_Fusion(object):
    def __init__(self):
        pass

    def __call__(self, img_ir, img_vis, img_mask,  boxes):
        # if random.random() < 0.3:
        #     img, boxes = colorJitter(img, boxes)
        if random.random() < 0.5:
            img_ir, img_vis, img_mask, boxes = random_rotation2(img_ir, img_vis, img_mask, boxes)
        if random.random() < 0.5:
            img_ir, img_vis, img_mask, boxes = random_crop_resize2(img_ir, img_vis, img_mask, boxes)
...
def random_rotation2(img_ir, img_vis,img_mask, boxes, degree=10):
    d = random.uniform(-degree, degree)
    w, h = img_ir.size
    rx0, ry0 = w / 2.0, h / 2.0
    img_ir = img_ir.rotate(d)
    img_vis = img_vis.rotate(d)
    img_mask = img_mask.rotate(d)
    a = -d / 180.0 * math.pi
...
def random_crop_resize2(img_ir, img_vis, img_mask, boxes, crop_scale_min=0.2, aspect_ratio=[3./4, 4./3], remain_min=0.7, attempt_max=10):
    success = False
    boxes = torch.from_numpy(boxes)
    for attempt in range(attempt_max):
        # choose crop size
        area = img_ir.size[0] * img_ir.size[1]
        target_area = random.uniform(crop_scale_min, 1.0) * area
        aspect_ratio_ = random.uniform(aspect_ratio[0], aspect_ratio[1])
        w = int(round(math.sqrt(target_area * aspect_ratio_)))
        h = int(round(math.sqrt(target_area / aspect_ratio_)))
        if random.random() < 0.5:
            w, h = h, w
        # if size is right then random crop
        if w <= img_ir.size[0] and h <= img_ir.size[1]:
            x = random.randint(0, img_ir.size[0] - w)
            y = random.randint(0, img_ir.size[1] - h)
            # check
            crop_box = torch.FloatTensor([[x, y, x + w, y + h]])
            inter = _box_inter(crop_box, boxes) # [1,N] N can be zero
            box_area = (boxes[:, 2]-boxes[:, 0])*(boxes[:, 3]-boxes[:, 1]) # [N]
            ...
            if box_remain.shape[0] != 0:
                ...
                break
    if success:
        img_ir = img_ir.crop((x, y, x+w, y+h))
        img_vis = img_vis.crop((x, y, x + w, y + h))
        img_mask = img_mask.crop((x, y, x + w, y + h))
        boxes -= torch.Tensor([x,y,x,y])
        boxes[:,1::2].clamp_(min=0, max=h-1)
        boxes[:,0::2].clamp_(min=0, max=w-1)
        # ow, oh = (size, size)
