...
from model.fcos_fusion import FCOSDetector


import numpy as np
from dataset.VOC_dataset_Fusion import VOCDataset
...
def eval_detection_mAP(model):
    data_set = VOCDataset(root_dir='E:\\fusion_attack\\Fusion_Detection\\M3FD_Detection\\', resize_size=[768,1024],
                            split='test', use_difficult=False, is_train=False, augment=None)
    print("INFO===>eval dataset has %d imgs"%len(data_set))
    eval_loader=torch.utils.data.DataLoader(data_set,batch_size=1,shuffle=False,collate_fn=data_set.collate_fn)
    ...
    pred_scores = []
    num = 0
    for img_ir, img_vis, name, boxes, classes in eval_loader:
        img_ir = img_ir.cuda()
        img_vis = img_vis.cuda()

        with torch.no_grad():
            _, scores1, classes1, boxes1=model.forward_object(img_ir,img_vis,None,None)

            pred_boxes.append(boxes1[0].cpu().numpy())
            pred_classes.append(classes1[0].cpu().numpy())
            pred_scores.append(scores1[0].cpu().numpy())
    ...
    all_AP=eval_ap_2d(gt_boxes,gt_classes,pred_boxes,pred_classes,pred_scores,0.5,len(data_set.CLASSES_NAME))
    

    print("all classes AP=====>\n")    
    for key,value in all_AP.items():
        print('ap for {} is {}'.format(data_set.id2name[int(key)],value))
    mAP=0.
    for class_id,class_mAP in all_AP.items():
        mAP+=float(class_mAP)
        print(class_id,class_mAP)
    mAP/=(len(data_set.CLASSES_NAME)-1)
    print("mAP=====>%.3f\n"%mAP)
    
if __name__ == "__main__":
    '''
        test detection and compute mAP
    '''
    ...
    print('| Testing detection on GPU #%d with pytorch' % (args.gpu))

    model = Network(None, mode='inference', config=DefaultConfig()).cuda()
    model.load_state_dict(torch.load("./checkpoint/model_meta_50.pth"),strict=False)
    print("===>success loading model")
    eval_detection_mAP(model)